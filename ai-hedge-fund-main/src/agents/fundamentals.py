...
def fundamentals_analyst_agent(state: AgentState, agent_id: str = "fundamentals_analyst_agent"):
    ...
    for ticker in tickers:
        ...
        progress.update_status(agent_id, ticker, "Calculating final signal (CoT)")

        # ── Chain of Thought (CoT) Structured Reasoning ──────────────────────
        # Adım 1: Makro → Adım 2: Sektör → Adım 3: Hisse Temelleri → Adım 4: Karar
        cot_reasoning = {
            "step_1_macro": "Overall market fundamentals assessed via profitability metrics",
            "step_2_sector": f"Sector analysis from growth signals: {reasoning.get('growth_signal', {}).get('details', 'N/A')}",
            "step_3_stock_fundamentals": f"Financial health: {reasoning.get('financial_health_signal', {}).get('details', 'N/A')}",
            "step_4_decision": "",
        }
        ...
        if bullish_signals > bearish_signals:
            ...
            cot_reasoning["step_4_decision"] = f"Bullish: {bullish_signals}/{len(signals)} sub-signals positive"
        elif bearish_signals > bullish_signals:
            overall_signal = "bearish"
            cot_reasoning["step_4_decision"] = f"Bearish: {bearish_signals}/{len(signals)} sub-signals negative"
        else:
            overall_signal = "neutral"
            cot_reasoning["step_4_decision"] = "Neutral: mixed fundamental evidence"

        # Calculate confidence level
        total_signals = len(signals)
        confidence = round(max(bullish_signals, bearish_signals) / total_signals, 2) * 100

        # CoT reasoning'i mevcut reasoning'e ekle
        reasoning["chain_of_thought"] = cot_reasoning
